...
public enum HostileUnitEnum {
     ...
     */
    public static HostileUnitEnum fromHostileUnit(@NonNull HostileUnit hostileUnit) {
        if (hostileUnit instanceof Infantry) return INFANTRY;
        if (hostileUnit instanceof Harvester) return HARVESTER;
        if (hostileUnit instanceof BossUnit) return BOSS_UNIT;
     ...
     */
    @NotNull
    public HostileUnit toHostileUnit(@NonNull Vector2 spawnPoint, HostileUnitController hostileUnitController) {
        switch (this) {
            case INFANTRY -> {
                if (hostileUnitController == null) return new Infantry(spawnPoint);
                return new Infantry(spawnPoint, hostileUnitController);
            }
            case BOSS_UNIT -> {
                if (hostileUnitController == null) return new BossUnit(spawnPoint);
                return new BossUnit(spawnPoint, hostileUnitController);
            }
            case HARVESTER -> {
                if (hostileUnitController == null) return new Harvester(spawnPoint);
