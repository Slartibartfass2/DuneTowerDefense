...
public final class HostileUnitController implements PropertyChangeListener {
    ...
    public void propertyChange(@NonNull PropertyChangeEvent evt) {
        if (evt.getPropertyName() == null || evt.getSource() == null || !(evt.getSource() instanceof HostileUnit hostileUnit))
            throw new IllegalStateException("Property name and source must be non null and source must be a hostile unit");
        
        switch (evt.getPropertyName()) {
            // Event when hostile unit is created
            case CREATE_EVENT_NAME -> handleCreateEvent(evt.getNewValue(), hostileUnit);
            ...
            case UPDATE_EVENT_NAME -> handleUpdateEvent(evt.getOldValue(), evt.getNewValue(), hostileUnit);
    ...
    private void handleShowEvent(@NonNull HostileUnit hostileUnit) {
