...
public class EntityController implements PropertyChangeListener {
    ...
    public void propertyChange(@NonNull PropertyChangeEvent evt) {
        if (evt.getPropertyName() == null || evt.getSource() == null || !(evt.getSource() instanceof Entity entity))
            throw new IllegalStateException("Property name and source must be non null and source must be an entity");
        
        switch (evt.getPropertyName()) {
            // Event when entity is created
            case CREATE_EVENT_NAME -> handleCreateEvent(evt.getNewValue(), entity);
            ...
            case UPDATE_EVENT_NAME -> handleUpdateEvent(evt.getNewValue(), entity);
    ...
    private void handleCreateEvent(@NonNull Object newValue, @NonNull Entity entity) {
        ...
        if (entity instanceof Tower tower) gameModel = assetLoader.getTowerGameModel(TowerEnum.fromTower(tower));
        else if (entity instanceof Portal portal) gameModel = assetLoader.getPortalGameModel(portal);
        else return;
    ...
    private void handleShowEvent(@NonNull Entity entity) {
        if (!entityModelMap.containsKey(entity)) throw new IllegalStateException(ENTITY_NOT_REGISTERED_MESSAGE);
    ...
    private void handleUpdateEvent(@NonNull Object newValue, @NonNull Entity entity) {
        ...
        else gameModel.rePosition(gameModelPosition);
        entityModelMap.put(entity, gameModel);
    }
    
    private void handleToDebrisEvent(@NonNull Entity entity) {
        if (!entityModelMap.containsKey(entity)) throw new IllegalStateException(ENTITY_NOT_REGISTERED_MESSAGE);
        if (!(entity instanceof Tower tower)) throw new IllegalArgumentException("Entity must be a tower");
    ...
    private void handleDestroyEvent(@NonNull Entity entity) {
        if (!entityModelMap.containsKey(entity)) throw new IllegalStateException(ENTITY_NOT_REGISTERED_MESSAGE);
    ...
    public float getRotation(@NonNull Entity entity) {
        if (!entityModelMap.containsKey(entity)) throw new IllegalStateException(ENTITY_NOT_REGISTERED_MESSAGE);
