...
public final class EntityGraph implements IndexedGraph<EntityNode> {
     ...
     */
    public EntityGraph(@NonNull Entity[][] grid) {
        ...
        for (int x = 0; x < gridWidth; x++) {
            for (int y = 0; y < gridHeight; y++) {
                ...
                if (x < gridWidth - 1) nodeIndex = addConnection(grid, nodeGrid, currentNode, x + 1, y, nodeIndex);
                
                // Add vertical connection
                if (y < gridHeight - 1) nodeIndex = addConnection(grid, nodeGrid, currentNode, x, y + 1, nodeIndex);
            }
        }
    }
    
    /**
     * Adds a bidirectional connection between the <code>currentNode</code> and the node with the specified position. Also
     * creates new node with the index <code>nodeIndex</code> if there's no node with the specified position.
     *
     ...
     */
    private int addConnection(@NonNull Entity[][] grid, @NonNull EntityNode[][] nodeGrid, @NonNull EntityNode currentNode, int nextX, int nextY, int nodeIndex) {
    ...
    /**
     * Adding a bidirectional connection between <code>fromNode</code> and <code>toNode</code> to the <code>connectionsMap</code>.
     *
    ...
    private void addConnection(@NonNull EntityNode fromNode, @NonNull EntityNode toNode) {
        ...
        if (fromNode.getEntity() instanceof Tower || toNode.getEntity() instanceof Tower) return;
        ...
        if (!connectionsMap.containsKey(fromNode)) connectionsMap.put(fromNode, new Array<>());
        ...
        if (!connectionsMap.containsKey(toNode)) connectionsMap.put(toNode, new Array<>());
    ...
    private @Nullable EntityNode findNode(int x, int y) {
        for (var entityNode : new Array.ArrayIterator<>(entityNodes).iterator()) {
            if (entityNode.getX() == x && entityNode.getY() == y) return entityNode;
        }
        return null;
    }
    ...
    public Array<Connection<EntityNode>> getConnections(@NonNull EntityNode fromNode) {
        if (connectionsMap.containsKey(fromNode)) return connectionsMap.get(fromNode);
        return new Array<>(0);
    }
}
